...
tmux send-keys -t $session:0 "roslaunch sar_simulations no_gui.launch \
    drone_id:=$drone_namespace " C-m

sleep 5

tmux new-window -t $session:1 -n 'slam'
tmux send-keys -t $session:1 "roslaunch sar_SLAM hector_slam.launch \
    drone_id:=$drone_namespace " C-m

tmux new-window -t $session:2 -n 'move_base'
tmux send-keys -t $session:2 "roslaunch sar_SLAM move_base.launch  \
    drone_id:=$drone_namespace " C-m
